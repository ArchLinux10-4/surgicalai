...
def _clean_history_content(content: str) -> str:
    ...
    if content.startswith("__COMPACTION_EVENT__:"):
        ...
        # NOTE: this copy of pipeline.py is a known-dead duplicate (never
        # imported); kept in sync only for hygiene per prior audit.
        try:
    ...
    try:
        ...
        for resolve_round in range(MAX_SYMBOL_RETRIES + 1):
            ...
            try:
                ...
                # The model may switch from <surgical_edit> to <new_file> when
                # the target file has no parseable symbols (e.g. SVG, plain HTML).
                # Capture both so neither is silently dropped.
                new_pending = []
                _cs, _cb = "normal", ""
                for char in corr_text:
                    if _cs == "normal":
                        ...
                        _cb += char
                        if _cb.endswith(FILE_CLOSE):
                            new_file_blocks_raw.append(_cb[:-len(FILE_CLOSE)])
                            _cb, _cs = "", "normal"
