...
# Cheap, explicit cues that the user wants an agentic task breakdown.
# We gate the (extra) planner call on these so ordinary single-step prompts
# flow through the existing pipeline path untouched — zero added latency/cost.
_PLAN_CUE_PATTERNS = [
    r"\bcreate (?:a )?tasks?\b",
    r"\bmake (?:a )?tasks?\b",
    ...
    r"\bgenerate tasks?\b",
    r"\bplan (?:out )?(?:the )?(?:work|tasks?|steps?)\b",
    r"\bdo all of (?:these|the following)\b",
    r"\bone by one\b",
    r"\bstep by step\b",
]
